...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=ks; k<=ke; k++) {
	...
	if(rad < 5.0*Radius_Earth){
          den_back = da;
          espec_back = ea;
        } else {
          den_back = std::max(da/100.0, da/(1.0+99.0*SQR((rad-5.0*Radius_Earth)/(35.0*Radius_Earth))));
          //espec_back = std::min(ea, ea + (4.0*espec_floor_/(35.0*R_E))*(r-5.0*R_E));
	  espec_back = ea;
        }
