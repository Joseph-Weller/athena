  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        ...
        if (rad_1 <= rad_max_1*1.3) {  // inside planetary body 1 body+atmosphere
          if (rad_1 <= rad_max_1*0.97) {  // inside planetary body 1
            ...
            den = (dc_1*1.0/(PI*0.97) *
                   std::sin((PI*0.97))*std::pow(rad_max_1*0.97/rad_1,15.0));
