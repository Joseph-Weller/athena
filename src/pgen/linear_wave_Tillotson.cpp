...
Real gam,gm1,iso_cs,vflow,espec0;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (PLANETARY_EOS) {
    //p0 = pmb->peos->PresFromRhoEs(d0, espec0);
    p0 = 3.975e10;
    //cs of 316915.4 
    h0 = ((espec0*d0 + 0.5*d0*(u0*u0 + v0*v0+w0*w0)) + p0)/d0;
  }
...
void Eigensystem(const Real d, const Real v1, const Real v2, const Real v3,
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      //--- Tillotson Implementation ---

    } else if (PLANETARY_EOS) {
      Real vsq = v1*v1 + v2*v2 + v3*v3;
      Real gm1_sub = p0/(espec0*d0);
      Real asq = gm1_sub*std::max((h-0.5*vsq), TINY_NUMBER);
      Real a = std::sqrt(asq);
      ...
      // Left-eigenvectors, stored as ROWS (eq. B4)
      Real na = 0.5/asq;
      left_eigenmatrix[0][0] = na*(0.5*gm1_sub*vsq + v1*a);
      left_eigenmatrix[0][1] = -na*(gm1_sub*v1 + a);
      left_eigenmatrix[0][2] = -na*gm1_sub*v2;
      left_eigenmatrix[0][3] = -na*gm1_sub*v3;
      left_eigenmatrix[0][4] = na*gm1_sub;
      ...
      Real qa = gm1_sub/asq;
      left_eigenmatrix[3][0] = 1.0 - na*gm1_sub*vsq;
      left_eigenmatrix[3][1] = qa*v1;
      left_eigenmatrix[3][2] = qa*v2;
      left_eigenmatrix[3][3] = qa*v3;
      left_eigenmatrix[3][4] = -qa;

      left_eigenmatrix[4][0] = na*(0.5*gm1_sub*vsq - v1*a);
      left_eigenmatrix[4][1] = -na*(gm1_sub*v1 - a);
      left_eigenmatrix[4][2] = left_eigenmatrix[0][2];
      left_eigenmatrix[4][3] = left_eigenmatrix[0][3];
      left_eigenmatrix[4][4] = left_eigenmatrix[0][4]; 
